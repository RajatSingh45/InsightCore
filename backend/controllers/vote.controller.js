...
const voteOnPost = async (req, res) => {
  try {
    ...
    const existingVote=await voteModel.findOne({user:userId,post:postId})
    ...
    await postModel.findByIdAndUpdate(postId, { score: newScore }, { new: true });
     
    res.status(200).json({success:true,message:"Successfully voted on post",existingVote})
  } catch (error) {
